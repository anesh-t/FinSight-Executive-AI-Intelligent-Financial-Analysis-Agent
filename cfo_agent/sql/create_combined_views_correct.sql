-- ============================================================================
-- CREATE COMBINED VIEWS - USING ONLY EXISTING COLUMNS
-- Run this in Supabase SQL Editor
-- ============================================================================
-- Based on actual schema verification:
-- - fact_financials: revenue, net_income, eps, r_and_d_expenses, sg_and_a_expenses, etc.
-- - vw_ratios_quarter: gross_margin, operating_margin, net_margin, roe, roa, etc.
-- - vw_stock_prices_quarter: open_price, close_price, avg_price, return_qoq, etc.
-- - vw_macro_quarter: gdp, cpi, unemployment_rate, fed_funds_rate, etc.
-- - vw_macro_sensitivity_rolling: betas
...
SELECT 
    ...
    -- Ratios (from vw_ratios_quarter - note: uses rnd_to_revenue, sgna_to_revenue)
    r.gross_margin, r.operating_margin, r.net_margin,
    r.roe, r.roa,
    r.debt_to_equity, r.debt_to_assets,
    r.rnd_to_revenue as rd_intensity,  -- Renamed
    r.sgna_to_revenue as sga_intensity,  -- Renamed
    
    -- Stock (from vw_stock_prices_quarter - note: NO _quarter suffix)
...
SELECT 
    ...
    -- Ratios (calculate from financials - mv_ratios_annual doesn't exist)
    CASE WHEN fa.revenue_annual > 0 
        THEN fa.gross_profit_annual / fa.revenue_annual 
        ELSE NULL END as gross_margin_annual,
    CASE WHEN fa.revenue_annual > 0 
        THEN fa.operating_income_annual / fa.revenue_annual 
        ELSE NULL END as operating_margin_annual,
    CASE WHEN fa.revenue_annual > 0 
        THEN fa.net_income_annual / fa.revenue_annual 
        ELSE NULL END as net_margin_annual,
    CASE WHEN fa.equity_eoy > 0 
        THEN fa.net_income_annual / fa.equity_eoy 
        ELSE NULL END as roe_annual,
    CASE WHEN fa.total_assets_eoy > 0 
        THEN fa.net_income_annual / fa.total_assets_eoy 
        ELSE NULL END as roa_annual,
    CASE WHEN fa.revenue_annual > 0 
        THEN fa.r_and_d_expenses_annual / fa.revenue_annual 
        ELSE NULL END as rd_intensity_annual,
    CASE WHEN fa.revenue_annual > 0 
        THEN fa.sgna_annual / fa.revenue_annual 
        ELSE NULL END as sga_intensity_annual,
